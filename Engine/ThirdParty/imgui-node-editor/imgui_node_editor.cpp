...
ed::EditorAction::AcceptResult ed::NavigateAction::Accept(const Control& control)
{
    ...
    if (Editor->CanAcceptUserInput() && Alice_IsKeyPressed(GetKeyIndexForF()) && Editor->AreShortcutsEnabled())
    {
...
ed::EditorAction::AcceptResult ed::ShortcutAction::Accept(const Control& control)
{
    ...
    auto& io = ImGui::GetIO();
    if (io.KeyCtrl && !io.KeyShift && !io.KeyAlt && Alice_IsKeyPressed(ImGuiKey_X))
        candidateAction = Cut;
    if (io.KeyCtrl && !io.KeyShift && !io.KeyAlt && Alice_IsKeyPressed(ImGuiKey_C))
        candidateAction = Copy;
    if (io.KeyCtrl && !io.KeyShift && !io.KeyAlt && Alice_IsKeyPressed(ImGuiKey_V))
        candidateAction = Paste;
    if (io.KeyCtrl && !io.KeyShift && !io.KeyAlt && Alice_IsKeyPressed(GetKeyIndexForD()))
        candidateAction = Duplicate;
    if (!io.KeyCtrl && !io.KeyShift && !io.KeyAlt && Alice_IsKeyPressed(ImGuiKey_Space))
...
ed::EditorAction::AcceptResult ed::DeleteItemsAction::Accept(const Control& control)
{
    ...
    auto& io = ImGui::GetIO();
    if (Editor->CanAcceptUserInput() && Alice_IsKeyPressed(ImGuiKey_Delete) && Editor->AreShortcutsEnabled())
    {
        auto& selection = Editor->GetSelectedObjects();
        if (!selection.empty())
        {
