...
extern "C"
{
    ...
    // ============================================================
// UI 스크립트 export 함수 추가
// ============================================================
    __declspec(dllexport) int Alice_GetDynamicUIScriptCount()
    {
        auto names = UIScriptFactory::GetRegisteredUIScriptNames();
        return static_cast<int>(names.size());
    }

    __declspec(dllexport) bool Alice_GetDynamicUIScriptName(int index, char* outName, int maxLen)
    {
        if (!outName || maxLen <= 0)
            return false;
    ...
    __declspec(dllexport) IUIScript* Alice_CreateDynamicUIScript(const char* name)
    {
        if (!name) return nullptr;
        auto ptr = UIScriptFactory::Create(name);
        return ptr.release();
    }
}
